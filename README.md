...
「網羅」ではなく「想定外の入力でクラッシュしたり、誤った情報を表示したりしないか」
を確認する目的で、既存ケースのコピーを一部改変した合成テストデータで4パターンを検証した。

| 確認パターン | 結果 |
|---|---|
| 未知のOpenFOAMソルバー名 | 問題なし（元々ソルバー名を分岐条件に使っていない） |
| 未知の乱流モデル名 | 問題なし（同上） |
| 未知のCalculiX材料モデルキーワード | バグを発見・修正 |
| 未知のCalculiX荷重条件キーワード | バグを発見・修正 |

見つかった主なバグ（修正済み）:

1. 材料プロパティの走査範囲が`*STEP`で区切られておらず、材料が1つしかないファイルでは
   ファイル末尾までスキャンしてしまう欠陥があった
2. ホワイトリストに無い材料プロパティ・荷重条件キーワードが、確認・警告なしに一覧から
   消えていた（→「該当しないと分かっているキーワード」の除外リスト方式に変更し、
   それ以外の未知語はraw値のまま表示するよう修正）
3. CalculiXの「静解析/動解析」「熱/構造」分類が、未対応の手続きキーワードを無条件に
   「静解析」「構造」に分類してしまう当てずっぽうのデフォルトになっていた
   （→既知の手続きにのみ分類し、未知は「不明」に倒すよう修正）
